...
namespace Model.Dao
{
    public class UserAccountDao
    {
        ...
        public bool Login(string username, string password)
        {
            ...
            var result = db.UserAccounts.Count(x => x.UserName == username && x.Password == password && x.Status==true&&x.Account==true);
            if(result > 0 )
            {
                return true;
            } else
        ...
        public bool LoginUser(string username, string password)
        {
            ...
            var result = db.UserAccounts.Count(x => x.UserName == username && x.Password == password && x.Status == true && x.Account == false); ;
            if (result > 0)
            {
                return true;
            }
